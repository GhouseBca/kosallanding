...
export default function HeroSection() {
  ...
  // FIXED: Perfect centering when clicking tag
  const scrollToCard = (id: string) => {
    const container = scrollRef.current;
    const card = cardRefs.current.get(id);
    if (!container || !card) return;

    setActiveId(id); // instant active state
  ...
  return (
    <section className="w-full h-fit bg-[#F8F8F8] dark:bg-[#161616] flex flex-col items-center justify-center py-[32px] md:py-[48px] lg:py-[64px] xl:py-[80px] gap-[24px] md:gap-[32px] lg:gap-[40px] overflow-hidden">
      ...
      <h1 className="font-inter font-semibold text-[26px] md:text-[44px] lg:text-[52px] xl:text-[64px] leading-[40px] md:leading-[64px] xl:leading-[72px] tracking-[-1%] text-center text-[#110C22] dark:text-white px-[24px] md:px-[40px] lg:px-[56px] xl:px-[64px]">
